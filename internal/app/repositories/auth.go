...
func (str *AuthRepo) Register(ctx context.Context, user entities.User) error {
	q := `INSERT INTO users(email, password) VALUES(?,?)`

	if _, err := str.db.ExecContext(ctx, q, user.Email, user.Password); err != nil {
		return err
	}
...
func (str *AuthRepo) GetUserByEmail(ctx context.Context, email string) (*entities.User, error) {
	var (
		user entities.User
	)
	...
	rows, err := str.db.QueryContext(ctx, q, email)
